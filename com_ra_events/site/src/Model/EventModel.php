...
class EventModel extends ItemModel implements CurrentUserInterface {
    ...
    protected function populateState() {
        ...
        if ((!$user->authorise('core.edit.state', 'com_ra_events')) && (!$user->authorise('core.edit', 'com_ra_events'))) {
        ...
        if (Factory::getApplication()->input->get('layout') == 'edit') {
            ...
            Factory::getApplication()->setUserState('com_ra_events.edit.event.id', $id);
        }
    ...
    public function getItem($id = null) {
        if ($this->_item === null) {
            ...
            if ($table && $table->load($id)) {
                ...
                if ($published = $this->getState('filter.published')) {
                    if (isset($table->state) && $table->state != $published) {
                        throw new \Exception(Text::_('COM_RA_EVENTS_ITEM_NOT_LOADED'), 403);
                    }
                }
            ...
            if (empty($this->_item)) {
                throw new \Exception(Text::_('COM_RA_EVENTS_ITEM_NOT_LOADED'), 404);
            }
        }
        ...
        /*
          if (isset($this->_item->event_type_id) && $this->_item->event_type_id != '') {
          if (is_object($this->_item->event_type_id)) {
          $this->_item->event_type_id = ArrayHelper::fromObject($this->_item->event_type_id);
          }

          $values = (is_array($this->_item->event_type_id)) ? $this->_item->event_type_id : explode(',', $this->_item->event_type_id);
          ...
          $this->_item->event_type = !empty($textValue) ? implode(', ', $textValue) : $this->_item->event_type_id;
          }
         */
        return $this->_item;
    }
